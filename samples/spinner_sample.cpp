...
private:
    bool do_init() {
        ...
        stage().actor(a2).set_parent(stage().actor(parent_));
        stage().actor(a3).set_parent(stage().actor(parent_));
    ...
    void do_step(double dt) {
        if(initialized()) {
            stage().actor(parent_).responsive_body().apply_angular_force_global(Vec3(0, 10, 0));
        }
    }
