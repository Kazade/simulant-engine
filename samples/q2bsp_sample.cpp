        ...
        window->camera(camera_id_)->set_perspective_projection(
            ...
            10000.0
...

                {
                    ...
                    auto world = stage->resources->get_mesh_with_alias("world_geometry");
                    auto lightmap_texture = stage->resources->mesh(world)->get<TextureID>("lightmap_texture_id");
                    auto rect_mat = lm_stage->resources->new_material_from_texture(lightmap_texture);
                    auto rectangle = lm_stage->resources->new_mesh_as_rectangle(0.25, 0.25, Vec2(), rect_mat);
