...
parser.add_argument(
    ...
    type=str,
    nargs=1,
    help="The output source file for the generated test main()",
)  # noqa
parser.add_argument(
    "test_files",
    type=str,
    nargs="+",
    help="The list of C++ files containing your tests",
)  # noqa
parser.add_argument(
...
parser.add_argument(
    "--list-tests",
    help="Don't generate the tests, simply output the discovered tests",
    action="store_true",
    default=False,
)  # noqa
...
CLASS_REGEX = (
    r"\s*class\s+(\w+)\s*([\:|,]\s*(?:public|private|protected)\s+[\w|::]+\s*)*"  # noqa
)
...
def find_tests(files):
    ...
    while i < len(subclasses):
        ...
        if (
            "TestCase" in subclass_names
            or "SimulantTestCase" in subclass_names
            or any(x[1] in subclasses[i][2] for x in test_case_subclasses)
        ):
...
def main():
    ...
    includes = "\n".join(
        [INCLUDE_TEMPLATE % {"file_path": x} for x in set([y[0] for y in testcases])]
    )
    registrations = []
    ...
    for path, class_name, superclasses, funcs in testcases:
        ...
        members = ", ".join(
            [BIND_TEMPLATE % {"class_name": class_name, "func": x} for x in funcs]
        )
        names = ", ".join(['"%s::%s"' % (class_name, x) for x in funcs])

        registrations.append(
            REGISTER_TEMPLATE
            % {"class_name": class_name, "members": members, "names": names}
        )

    registrations = "\n".join(registrations)

    final = MAIN_TEMPLATE % {"registrations": registrations, "includes": includes}
