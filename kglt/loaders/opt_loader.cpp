...
void OPTLoader::into(Loadable& resource) {
    ...
    //We read those here
    std::vector<int32_t> mesh_header_offsets;
    for(uint16_t i = 0; i < main_jump_header.mesh_header_offset_count; ++i) {
        int32_t offset;
        file.read((char*)&offset, sizeof(int32_t));
        mesh_header_offsets.push_back(offset - main_header.global_offset); //Again, add the global offset
    }

    MainMeshHeader main_mesh_header;
    file.read((char*)&main_mesh_header, sizeof(MainMeshHeader));

    //Do the same dance as the mesh header offsets here
    main_mesh_header.offset_to_jump_offsets -= main_header.global_offset;
    file.seekg(main_mesh_header.offset_to_jump_offsets, std::ios_base::beg);

    std::queue<int32_t> block_offsets;
    for(uint16_t i = 0; i < main_mesh_header.jump_count; ++i) {
        int32_t offset;
        file.read((char*)&offset, sizeof(int32_t));
        block_offsets.push(offset - main_header.global_offset); //Again, add the global offset
    }
    ...
    while(!block_offsets.empty()) {
        ...
        switch(data_header.type) {
            case DataBlockTypes::EMBEDDED_TEXTURE: {
                ...
                file.read((char*)&texture_data_block, sizeof(EmbeddedTextureDataBlock));
                texture_data_block.offset_to_palette_data_offset -= main_header.global_offset;
                texture_data_block.offset_to_alpha_mask_block -= main_header.global_offset;

                //Read the image data following the data block
                //std::vector<uint8_t> image_data(texture_data_block.data_size);
                //file.read((char*)&image_data, sizeof(uint8_t) * texture_data_block.data_size);

                std::string texture_name = std::string(texture_data_block.texture_name, texture_data_block.texture_name + 8);
                L_DEBUG(unicode("Found texture with ID {0} and name '{1}'").format(texture_data_block.texture_id, texture_name).encode());

                Texture new_texture;
                new_texture.name = texture_name;
/*                new_texture.width = texture_data_block.width;
                new_texture.height = texture_data_block.height;
                new_texture.bytes_per_pixel = 3; //FIXME:

                uint16_t palette_data[256 * 16];
                file.seekg(texture_data_block.offset_to_palette_data);
                file.read((char*)&palette_data, sizeof(uint16_t) * 256 * 16);
                ...
                for(int32_t i = 0; i < new_texture.width * new_texture.height; ++i) {
                    uint32_t palette_index = image_data[i];
                    ...
                    new_texture.data[i * 3] = r;
                    new_texture.data[i * 3 + 1] = g;
                    new_texture.data[i * 3 + 2] = b;
                }*/
            ...
            case DataBlockTypes::FACE: {
                ...
                for(int32_t j = 0; j < face_data_block.face_count; ++j) {
                    ...
                    assert(vertex_normal_indexes[0] < vertex_normals.size());
                    assert(vertex_normal_indexes[1] < vertex_normals.size());
                    assert(vertex_normal_indexes[2] < vertex_normals.size());
                    if(vertex_normal_indexes.size() == 4) {
                        assert(vertex_normal_indexes[3] < vertex_normals.size());
                    }
                    ...
                    if(vertex_indices.size() == 3) {
                        ...
                        t1.normal_indexes[0] = vertex_normal_indexes[0];
                        t1.normal_indexes[1] = vertex_normal_indexes[2];
                        t1.normal_indexes[2] = vertex_normal_indexes[3];
    ...
    for(Texture tex: textures) {
        ...
        texture_submesh[tex.name] = mesh->new_submesh(MaterialID(), MESH_ARRANGEMENT_TRIANGLES, false);
    }
