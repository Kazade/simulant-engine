...
public:
     ...
     * that an alloc row (which may have a blank source, since some assets
     * don't get their source path set until after construction) can still
     * be matched up with its corresponding dealloc row. */
...
inline void log_memory_event(MemoryLogEvent event, const char* type,
                              uint64_t id, const std::string& source,
                              const std::string& name) {
    if(memory_logger) {
        memory_logger->log(event, type, id, source, name);
    }
}
