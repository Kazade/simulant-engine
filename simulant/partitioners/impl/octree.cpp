...
Octree::Octree(Stage *stage,
    ...
    debug_material_ = stage_->assets->new_material_from_file(smlt::Material::BuiltIns::DIFFUSE_ONLY);
    auto mat = stage_->assets->material(debug_material_);
    mat->first_pass()->set_diffuse(smlt::Colour(0.29, 0.82, 1.0, 0.4));
    mat->first_pass()->set_blending(smlt::BLEND_ONE_ONE_MINUS_ALPHA);
    mat->first_pass()->set_polygon_mode(smlt::POLYGON_MODE_LINE);
    mat->first_pass()->set_cull_mode(smlt::CULL_MODE_NONE);
}
