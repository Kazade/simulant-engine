...
namespace std {
  ...
  template <typename... Ts>
  struct hash<smlt::detail::enabled_type<
      smlt::variant<Ts...>,
      smlt::lib::enable_if_t<smlt::lib::all<smlt::detail::hash::is_enabled<
          smlt::lib::remove_const_t<Ts>>()...>::value>>> {
    using argument_type = smlt::variant<Ts...>;
    using result_type = std::size_t;

    inline result_type operator()(const argument_type &v) const {
      using smlt::detail::visitation::variant;
      std::size_t result =
...
#ifdef MPARK_GENERIC_LAMBDAS
                    [](const auto &alt) {
                      using alt_type = smlt::lib::decay_t<decltype(alt)>;
                      using value_type = smlt::lib::remove_const_t<
    ...
    struct hasher {
      ...
      inline std::size_t operator()(const Alt &alt) const {
        using alt_type = smlt::lib::decay_t<Alt>;
        using value_type =
            smlt::lib::remove_const_t<typename alt_type::value_type>;
        return hash<value_type>{}(alt.value);
      }
    };
#endif
  ...
  struct hash<smlt::monostate> {
    using argument_type = smlt::monostate;
    using result_type = std::size_t;
