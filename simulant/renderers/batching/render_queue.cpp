...
void RenderQueue::insert_renderable(Renderable&& src_renderable) {
    ...
    assert(src_renderable.vertex_data->gpu_buffer() ||
           (src_renderable.vertex_data && src_renderable.vertex_data->count()));

    if(!src_renderable.vertex_data->gpu_buffer() &&
       !src_renderable.vertex_data->count()) {
