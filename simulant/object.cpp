...
void MoveableObject::set_absolute_position(float x, float y, float z) {
    ...
    smlt::Vec3 parent_pos;
    if(has_parent()) {
        parent_pos = parent()->as<SceneNode>()->position();
    }

    set_relative_position(smlt::Vec3(x, y, z) - parent_pos);
}
...
void MoveableObject::rotate_around(const smlt::Vec3& axis, const smlt::Degrees& degrees) {
    if(rotation_locked_) {
        return;
    }
