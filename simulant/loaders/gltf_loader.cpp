...
void process_positions(const BufferInfo& buffer_info, JSONIterator&,
                       smlt::MeshPtr& final_mesh, const VertexFormat& spec) {
...
#endif
    auto ccount = spec.attr(VERTEX_ATTR_NAME_POSITION)->component_count();
    if(ccount == 2) {
...
void process_colors(const BufferInfo& buffer_info, JSONIterator& js,
    ...
    auto ccount = spec.attr(VERTEX_ATTR_NAME_COLOR)->component_count();
    auto type = spec.attr(VERTEX_ATTR_NAME_COLOR)->type;
...
void process_normals(const BufferInfo& buffer_info, JSONIterator& js,
    ...
    auto ccount = spec.attr(VERTEX_ATTR_NAME_NORMAL)->component_count();
...
static smlt::MeshPtr load_mesh(AssetManager* assets, JSONIterator& js,
    ...
    const std::map<TypeKey,
                   std::pair<VertexAttributeArrangement, VertexAttributeType>>
        lookup = {
            {TypeKey(FLOAT,          "VEC2"), std::make_pair(VERTEX_ATTR_ARRANGEMENT_XY,
             VERTEX_ATTR_TYPE_FLOAT)         },
            {TypeKey(FLOAT,          "VEC3"), std::make_pair(VERTEX_ATTR_ARRANGEMENT_XYZ,
             VERTEX_ATTR_TYPE_FLOAT)         },
            {TypeKey(FLOAT,          "VEC4"),
             std::make_pair(VERTEX_ATTR_ARRANGEMENT_XYZW,
             VERTEX_ATTR_TYPE_FLOAT)         },
            // {TypeKey(FLOAT, "SCALAR"), VERTEX_ATTRIBUTE_1F},
            {TypeKey(UNSIGNED_BYTE,  "VEC4"),
             std::make_pair(VERTEX_ATTR_ARRANGEMENT_XYZW,
             VERTEX_ATTR_TYPE_UNSIGNED_BYTE) },
            {TypeKey(UNSIGNED_SHORT, "VEC4"),
             std::make_pair(VERTEX_ATTR_ARRANGEMENT_XYZW,
             VERTEX_ATTR_TYPE_UNSIGNED_SHORT)},
    };

    auto process_attribute = [&](int acc_id)
        -> optional<
            std::pair<VertexAttributeArrangement, VertexAttributeType>> {
        if(acc_id == -1) {
            return no_value;
    ...
    for(auto& primitive: primitives) {
        ...
        auto spec = VertexFormatBuilder();

        if(pos) {
            spec = spec.add(VERTEX_ATTR_NAME_POSITION, pos.value().first,
                            pos.value().second);
        }

        if(norm) {
            spec = spec.add(VERTEX_ATTR_NAME_NORMAL, norm.value().first,
                            norm.value().second);
        }

        if(tex) {
            spec = spec.add(VERTEX_ATTR_NAME_TEXCOORD_0, norm.value().first,
                            norm.value().second);
        }

        if(diff) {
            spec = spec.add(VERTEX_ATTR_NAME_COLOR, diff.value().first,
                            diff.value().second);
        }

        if(!final_mesh) {
            final_mesh = assets->create_mesh(spec);
        } else if(final_mesh->vertex_data->vertex_format() != spec) {
        ...
        if(primitive.color_id >= 0) {
            ...
            process_colors(buffer_info, js, final_mesh, spec);
        }
