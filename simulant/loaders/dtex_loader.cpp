...
static bool load_dtex(std::istream& stream, Texture* result) {
    DTexHeader header;
...
bool DTEXLoader::do_load(std::shared_ptr<FileIfstream> stream, Texture* result) {
    return load_dtex(*stream, result);
}

bool DTEXLoader::do_load(const std::vector<uint8_t>& data, Texture* result) {
