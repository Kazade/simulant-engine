...
public:
    ...
    virtual void move_to(const smlt::Vec3& pos) = 0;
    virtual void move_to(const smlt::Vec2& pos) = 0;
    virtual void move_to(float x, float y, float z) = 0;
    virtual void move_to(float x, float y) = 0;

    virtual void move_by(const smlt::Vec3& pos) = 0;
    virtual void move_by(const smlt::Vec2& pos) = 0;
    virtual void move_by(float x, float y, float z) = 0;
    virtual void move_by(float x, float y) = 0;

    virtual void rotate_to(const smlt::Degrees& angle) = 0;
    virtual void rotate_to(const smlt::Degrees& angle, float axis_x, float axis_y, float axis_z) = 0;
    virtual void rotate_to(const smlt::Degrees& angle, const smlt::Vec3& axis) = 0;
    virtual void rotate_to(const smlt::Quaternion& rotation) = 0;

    virtual void rotate_x(const smlt::Degrees& angle) = 0;
    virtual void rotate_y(const smlt::Degrees& angle) = 0;
    virtual void rotate_z(const smlt::Degrees& angle) = 0;
    ...
    virtual void rotate_around(const smlt::Vec3& axis, const smlt::Degrees& degrees) = 0;

    void rotate_global_x(const smlt::Degrees& degrees) {
        const static Vec3 X(1, 0, 0);
        rotate_around(X, degrees);
    }

    void rotate_global_y(const smlt::Degrees& degrees) {
        const static Vec3 Y(0, 1, 0);
        rotate_around(Y, degrees);
    }

    void rotate_global_z(const smlt::Degrees& degrees) {
        const static Vec3 Z(0, 0, 1);
        rotate_around(Z, degrees);
    }

    virtual void look_at(const smlt::Vec3& target) = 0;
