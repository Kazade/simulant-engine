...
void EventListener::handle_touch_begin(Core* core, TouchPointID touch_id, float normalized_x, float normalized_y, float pressure) {
    ...
    evt.coord = core->coordinate_from_normalized(normalized_x, normalized_y);

    on_touch_begin(evt);
}

void EventListener::handle_touch_end(Core* core, TouchPointID touch_id, float normalized_x, float normalized_y) {
    TouchEvent evt;
...
void EventListener::handle_touch_move(Core* core, TouchPointID touch_id, float normalized_x, float normalized_y, float dx, float dy) {
    TouchEvent evt;
...
void EventListener::handle_key_down(Core* core, KeyboardCode code, ModifierKeyState modifiers) {
    _S_UNUSED(core);
...
void EventListener::handle_key_up(Core* core, KeyboardCode code, ModifierKeyState modifiers) {
    _S_UNUSED(core);
